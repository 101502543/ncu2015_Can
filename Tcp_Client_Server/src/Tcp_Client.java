...
public class Tcp_Client implements Runnable{
	...
	//紀錄當下時槽
	int nowTimeSlot = 1;
	public Tcp_Client(String serverIP) throws IOException{
	...
	public void run(){
		...
		try {
			...
			timer_client.schedule(clientTimer,3000,3000);
			while(true){
				try {
	...
	/*private int getNextIndex(int treasure_index){
		...
		while(count < 3){
			if(!isHaveTreasure[treasure_index])
				return treasure_index;
			
			treasure_index = (treasure_index++)%3;
			count++;
		}
		return -1;
	}*/
...
class treasureTimer extends TimerTask{
	...
	public void run() {
		...
		try {
			...
			Tcp_Client.tcp_Client.outputStream.flush();
